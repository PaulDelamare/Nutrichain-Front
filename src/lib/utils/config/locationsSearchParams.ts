...
/**
 * Query params du listing des emplacements (onglet Configuration). On garde `tab=locations` dans
 * l'URL (onglets = navigation), les filtres partent à l'API et un nouveau filtre repart en page 1.
 */
export function locationsSearchParams(
...
export function locationsPageSizeParams(current: URLSearchParams, size: number): URLSearchParams {
	const params = new URLSearchParams(current);
	params.set('tab', 'locations');
